...
def loadData(name, num_components=None, preprocessing="standard"):
    data_path = "./HSI-datasets"
    if name  in ["IP", "DIP", "DIPr"]:
        data = sio.loadmat('./HSI-datasets/indian_pines.mat')['indian_pines']
        labels = sio.loadmat('./HSI-datasets/indian_pines_gt.mat')['indian_pines_gt']
    elif name == 'SV':
        data = sio.loadmat(os.path.join(data_path, 'salinas_corrected.mat'))['salinas_corrected']
        labels = sio.loadmat(os.path.join(data_path, 'salinas_gt.mat'))['salinas_gt']
    elif name  in ["UP", "DUP", "DUPr"]:
        data = sio.loadmat('./HSI-datasets/paviaU.mat')['paviaU']
        labels = sio.loadmat('./HSI-datasets/paviaU_gt.mat')['paviaU_gt']
    elif name == 'UH':
...
def load_split_data_fix(name, pixels, path_dset='/HSI-datasets'):
    data_path = os.path.join(path_dset)
    if name == "UH":
        ...
        y_train2 = sio.loadmat('./HSI-datasets/indianpines_disjoint_dset.mat')\
                                            ['GetBlock_gt']
        y_test = sio.loadmat('./HSI-datasets/indian_pines_gt.mat')['indian_pines_gt']
        ...
        y_train = sio.loadmat('./HSI-datasets/TRpavia_fixed.mat')['GetBlock'].reshape(-1)
        y_test = sio.loadmat('./HSI-datasets/TSpavia_fixed.mat')['dis_TestBlock_gt'].reshape(-1)
        if name == "DUP": pass
        else: X_train, X_test, y_train, y_test = select_samples(pixels, y_test, y_train)
