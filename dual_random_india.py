...
add_data = "india_16class_1034_rand1/"  ##########
print ("./Prob_MRF/"+add_data+"train_feature.txt")

learning_rate = 0.002
training_iters = 12000*1+50  ###########1034
batch_size = 1  ##########1034
display_step = 50
...
print (Prob_MRF_train_coordinate.shape)
...
Prob_MRF_train_labels = np.zeros([train_labels.shape[0], n_classes])#1034*16
...
weights = {
    ...
    'wc1': tf.Variable(tf.random_normal([1, 21, 1, 20])),
    #f_hight, f_width, input_channel, output_channel
    'wd1_1': tf.Variable(tf.random_normal([40*20, 100])),#通道800到100 图片大小*通道数 40为卷积后池化结果
...
def conv_lstm_net1(x, pixel_coordinate, weights, biases, dropout):
    ...
    conv1 = pool1[:,:,:40,:] #只取前40个max_pooling后的值（由于向上取整池化结果共有31个值）
    print ('after maxpool',conv1.get_shape())

    fc1 = tf.reshape(conv1, [-1, weights['wd1_1'].get_shape().as_list()[0]])#tf.reshape(conv1,[-1,800])
    print ("*********************************")
...
###################################################################################################################################################################
loss_mes = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=y))#reduce是求均值
loss_regularization = []
loss_regularization.append(tf.nn.l2_loss(weights['wd1_1']))#使用L2正则化
loss_regularization.append(tf.nn.l2_loss(weights['out']))#使用L2正则化
loss_regularization = tf.reduce_sum(loss_regularization)
cost = loss_mes+0.025*loss_regularization
###################################################################################################################################################################
...
#    # print ('type(probability):',type(probability))
    
#     np.savetxt("./Prob_MRF/"+add_data+"dual_outcome/"+"probability/"+i_test+"probability.txt", probability, '%.3f')
#     np.savetxt("./Prob_MRF/"+add_data+"dual_outcome/"+"probability/"+i_test+"probability_softmax.txt", probability_softmax, '%.3f')
    
#     labels_pred = np.argmax(probability, 1) #1代表行方向 取最大值的索引
#     #labels_pred_softmax = np.argmax(probability_softmax, 1) #1代表行方向 取最大值的索引
#     #labels_pred = np.column_stack((labels_pred, labels_pred_softmax))
#     #print ('labels_pred:',labels_pred)
#     #print ('labels_pred.shape:',labels_pred.shape)
   
#     np.savetxt("./Prob_MRF/"+add_data+"dual_outcome/"+"probability/"+i_test+"labels_pred.txt", labels_pred, '%d')
    
#     #weights_save = sess.run(weights)
#     #print weights_save
#     #biases_save = sess.run(biases)
#     #print biases_save
